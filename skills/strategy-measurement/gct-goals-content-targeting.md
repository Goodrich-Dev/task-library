---
name: gct-goals-content-targeting
...
A persistent, max-effort agent (Claude Fable 5 or comparable OpenAI/Google models) drafts the GCT brief for every asset before launch and loops until the Definition of done fully passes — numeric Goal, proof-backed Content, specific Targeting, and an alignment sentence that reads true — blocking launch rather than shipping at 90%.
It self-verifies by reading G, C, and T back as one sentence and fixing the weak corner before any spend.
Because briefs persist in memory, the MAA loop becomes a true memory cycle: each review judges performance against the originally stated goal (no retrofitting), classifies misses as G, C, or T failures versus prior campaigns, and logs a meta-article example each run.
